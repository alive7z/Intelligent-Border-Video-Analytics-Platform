          ...
          <table className="w-full min-w-[820px] text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-xs font-medium uppercase tracking-wide text-slate-500">
                <th className="px-5 py-3">Face Event</th>
            ...
            <tbody>
              {events.map((f) => (
                <tr key={f.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  <td className="px-5 py-3 font-medium text-navy-700">{f.id}</td>
                  <td className="px-5 py-3 text-slate-700">{f.trackId}</td>
                  <td className="px-5 py-3 text-slate-600">{f.cameraId}</td>
                  <td className="px-5 py-3 text-slate-600">{f.cameraName}</td>
          ...
          <div key={f.id} className="card p-4">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-navy-700">{f.id}</p>
