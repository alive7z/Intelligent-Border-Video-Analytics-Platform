            ...
            <tbody>
              {events.map((e) => (
                <tr key={e.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  <td className="px-5 py-3 font-medium text-blue-600">{e.plateNumber}</td>
                  <td className="px-5 py-3 text-secondary">{formatEventLabel(e.vehicleType) || "—"}</td>
                  <td className="px-5 py-3 text-secondary">{e.cameraId || "—"}</td>
                  <td className="px-5 py-3 text-secondary">{e.location || "—"}</td>
        ...
        {events.map((e) => (
          <div key={e.id} className="card p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-blue-600">{e.plateNumber}</p>
