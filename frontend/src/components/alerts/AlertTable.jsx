  ...
  return (
    <>
      ...
      <div className="space-y-3 md:hidden">
        {alerts.map((a) => (
          <div key={a.id} className="card border-l-2 border-l-blue-500 p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-blue-600">{a.id}</p>
                <p className="text-sm text-secondary">{formatEventLabel(a.eventType)}</p>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <Link
                to={`/alerts/${a.id}`}
                className="btn-focus inline-flex flex-1 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-secondary hover:bg-slate-50 dark:bg-slate-100"
              >
                View
              </Link>
              {showSave && (
