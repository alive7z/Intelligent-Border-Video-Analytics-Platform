        ...
        <ul className="divide-y divide-slate-100 text-sm">
          {events.slice(0, 5).map((e) => (
            <li key={e.id}>
              ...
              >
                <span className="min-w-0 font-medium text-blue-700 hover:underline">{e.cameraId}<span className="block text-xs font-normal text-slate-500">{e.correlation?.reason}{e.correlation?.confidence ? ` · Correlation confidence: ${e.correlation.confidence}` : ""}</span></span>
